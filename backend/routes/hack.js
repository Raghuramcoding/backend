...
router.post("/", (req, res) => {
  ...
  if (success) {
    ...
    db.prepare("UPDATE hackers SET successful_hacks = successful_hacks + 1, total_stolen = total_stolen + ?, updated_at = datetime('now') WHERE username = ?")
      .run(stolen, hackerName);
  } else {
